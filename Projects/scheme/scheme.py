...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        env.bindings[target.first] = do_lambda_form(Pair(expressions.first.rest, expressions.rest) , env)
        return target.first
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions.rest != nil:
        ...
        if test == False and test is not 0:
            return False
        else:
            expressions = expressions.rest
    return scheme_eval(expressions.first, env, True)
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        check_form(pair, 2, 2)
        f = Pair(pair.first, f)
        v = Pair(scheme_eval(pair.rest.first, env), v)
...
def do_define_macro(expressions, env):
    ...
    formals, parameters, body = expressions.first.first, expressions.first.rest, expressions.rest
    if not (scheme_symbolp(formals)):
        raise SchemeError()
    env.define(formals, MacroProcedure(parameters, body, env))
    return formals
    # END Problem 20
