...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        food = random_or_none(bee)
        if food != None:
            Insect.reduce_armor(food, food.armor)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            while check != None:
                if check.ant != None and check.ant.is_container and check.ant.contained_ant != None:
                    if not check.ant.contained_ant.doubled:
                        double(check.ant.contained_ant)
                if check.ant != None and not check.ant.doubled:
                    if check.ant.is_container and check.ant.contained_ant != None and not check.ant.contained_ant.doubled:
                        double(check.ant.contained_ant)
                    double(check.ant)
                check = check.exit
        # END Problem 13
